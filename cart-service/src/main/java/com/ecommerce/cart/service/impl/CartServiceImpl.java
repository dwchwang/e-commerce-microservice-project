...
public class CartServiceImpl implements CartService {
  ...
  @Override
  public CartResponse mergeGuestToUser(String guestSessionId, String userId) {
    String guestKey = "cart:guest:" + guestSessionId;
    String userKey = "cart:" + userId;

    Map<Object, Object> guestEntries = redisTemplate.opsForHash().entries(guestKey);
    if (guestEntries.isEmpty()) {
      return getCart(userKey);
    }

    // Merge: for each guest item, add to user cart (increment quantity if already
    // exists)
    for (Map.Entry<Object, Object> entry : guestEntries.entrySet()) {
      String productId = (String) entry.getKey();
      CartItem guestItem = readCartItem(entry.getValue());

      String existingJson = (String) redisTemplate.opsForHash().get(userKey, productId);
      if (existingJson != null) {
        CartItem existing = readCartItem(existingJson);
        existing.setQuantity(existing.getQuantity() + guestItem.getQuantity());
        redisTemplate.opsForHash().put(userKey, productId, writeCartItem(existing));
      } else {
        redisTemplate.opsForHash().put(userKey, productId, writeCartItem(guestItem));
      }
    }

    // Clear guest cart & set TTL for user cart
